...
async function selectCourses(existingCourses?: string[]): Promise<string[]> {
...
async function promptPayment(name: string, courses: string[], remainingBalance: number = 0): Promise<number> {
    const totalTuitionFee = courses.reduce((total, course) => total + tuitionFees[course], 0);
